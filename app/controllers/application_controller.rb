...
class ApplicationController < ActionController::Base
  ...
  def internal_error(e)
    ...
    if (e.is_a?(Pundit::AuthorizationNotPerformedError) || e.is_a?(Pundit::NotAuthorizedError))
      render :file => "#{Rails.root}/../not_authorized.html" #When warbler warbles it puts static html one level above Rails Root!
      return
    elsif e.is_a? Faraday::ConnectionFailed
      # thrown if Nexus is down.
      render :file => "#{Rails.root}/../nexus_not_available.html"
      return
    end
  ...
  def ensure_services_configured
    ...
    render :file => "#{Rails.root}/../not_configured.html" unless (application_server_configured && artifactory_configured  && build_server_configured)
    return
  end
