...
module PrismeUtilities
  ...
  def self.write_vuid_db
    ...
    json.merge!({'epoch_time_seconds' => Time.now.to_i})
   # json.merge!({'log_events_url' =>  PrismeUtilities::RouteHelper.route(:log_event_url, security_token: TokenSupport.instance.generate_security_token(preamble:LogEventsController::LOG_EVENT_PREAMBLE))})
    #above approach automatically escapes all params.  In most cases you want this, but not for our url safe security token
     json.merge!({'log_events_url' =>  PrismeUtilities::RouteHelper.route(:log_event_url)<< "?security_token=#{TokenSupport.instance.generate_security_token(preamble:LogEventsController::LOG_EVENT_PREAMBLE)}"})
    begin
